...
import renderCreate from '../../activitypub/renderer/create';
import renderAnnounce from '../../activitypub/renderer/announce';
import { countIf } from '../../prelude/array';
import * as url from '../../prelude/url';
import { Users, Notes } from '../../models';
import { makePaginationQuery } from '../api/common/make-pagination-query';
import { Brackets } from '../../activitypub/renderer/node_modules/typeorm';
...
export default async (ctx: Router.RouterContext) => {
	...
	const limit = 20;
	const partOf = `${server.url}/users/${userId}/outbox`;
...
export async function packActivity(note: Note): Promise<any> {
	if (note.renoteId && note.text == null && !note.hasPoll && (note.fileIds == null || note.fileIds.length == 0)) {
		const renote = await server.getters.findNote(note.renoteId).then(ensure);
		return renderAnnounce(renote.uri ? renote.uri : `${server.url}/notes/${renote.id}`, note);
	}
